...
namespace rb1_base_controller
{
    ...
    // Service SetOdometry
    bool RB1BaseController::setOdometrySrvCallback(robotnik_msgs::set_odometry::Request &request, robotnik_msgs::set_odometry::Response &response )
    {
		...
		ROS_INFO("%s::setOdometrySrvCallback: q.x=%5.2f q.y=%5.2f  q.z=%5.2f  q.w=%5.2f", controller_name_.c_str(), q.x(), q.y(), q.z(), q.w() );
    ...
    bool RB1BaseController::init(hardware_interface::VelocityJointInterface* hw,
            ...
            ros::NodeHandle &controller_nh)
    {
        ...
        wheel_base_ =  0.421; //0.569;
        track_width_ = 0.421; //0.543;
        wheel_diameter_ = 0.1524; //0.233;
        ...
        if(elevator_digital_output_up_ < 1){
			 ROS_WARN_STREAM_NAMED(controller_name_, controller_name_ << "::initController: param elevator_digital_output_up (=" << elevator_digital_output_up_  <<") has to be >= 1. Setting default value " << RB1_ELEVATOR_DEFAULT_OUTPUT_UP);
			elevator_digital_output_up_ = RB1_ELEVATOR_DEFAULT_OUTPUT_UP;
		}
        if(elevator_digital_output_down_ < 1){
			 ROS_WARN_STREAM_NAMED(controller_name_, controller_name_ << "::initController: param elevator_digital_output_down (=" << elevator_digital_output_down_  <<") has to be >= 1. Setting default value " << RB1_ELEVATOR_DEFAULT_OUTPUT_DOWN);
			elevator_digital_output_down_ = RB1_ELEVATOR_DEFAULT_OUTPUT_DOWN;
		}
        if(elevator_digital_input_up_ < 1){
			 ROS_WARN_STREAM_NAMED(controller_name_, controller_name_ << "::initController: param elevator_digital_input_up (=" << elevator_digital_input_up_  <<") has to be >= 1. Setting default value " << RB1_ELEVATOR_DEFAULT_INPUT_UP);
			elevator_digital_input_up_ = RB1_ELEVATOR_DEFAULT_INPUT_UP;
		}
        if(elevator_digital_input_down_ < 1){
			 ROS_WARN_STREAM_NAMED(controller_name_, controller_name_ << "::initController: param elevator_digital_input_down (=" << elevator_digital_input_down_  <<") has to be >= 1. Setting default value " << RB1_ELEVATOR_DEFAULT_INPUT_DOWN);
        ...
        controller_nh.param("elevator_action_command_timeout_msg", elevator_action_command_timeout_msg, ELEVATOR_ACTION_COMMAND_TIMEOUT_MSG);
        ...
        controller_nh.param<std::string>("joint/right_wheel_joint/name", joint_names_[RIGHT_TRACTION_JOINT], "rb1_base_right_wheel_joint");
        controller_nh.param<std::string>("joint/left_wheel_joint/name", joint_names_[LEFT_TRACTION_JOINT], "rb1_base_left_wheel_joint");
     ...
     *
     */
    bool RB1BaseController::setElevatorSrvCallback(robotnik_msgs::SetElevator::Request &req, robotnik_msgs::SetElevator::Response &res )
    {
      ROS_INFO("%s::setElevatorSrvCallback: action %d. Configured with has_elevator=%d, RAISE:%d, LOWER:%d", controller_name_.c_str(), 
      req.action.action, has_elevator_, robotnik_msgs::ElevatorAction::RAISE, robotnik_msgs::ElevatorAction::LOWER);
	  res.ret = false;

      if (!has_elevator_){
          ROS_ERROR("%s::setElevatorSrvCallback: controller with no elevator",  controller_name_.c_str());
		  return false;
      }
      
      if(inMotion()){
		  ROS_WARN("%s::setElevatorSrvCallback: robot in motion. Action not allowed!",  controller_name_.c_str());
		  return false;
	  }
	  
	  if(elevator_status_.state != robotnik_msgs::ElevatorStatus::IDLE){
		  ROS_WARN("%s::setElevatorSrvCallback: elevator not idle!",  controller_name_.c_str());
		  return false;
	  }
	  
      if(req.action.action == robotnik_msgs::ElevatorAction::RAISE or req.action.action == robotnik_msgs::ElevatorAction::LOWER){
          last_elevator_action_time = ros::Time::now();
      }else{
		  ROS_ERROR("%s::setElevatorSrvCallback: Action %d not allowed", controller_name_.c_str(), req.action.action);
		  return false;
	  }
    ...
    void RB1BaseController::update(const ros::Time& time, const ros::Duration& period)
    {
        ...
        //    ROS_WARN_STREAM_NAMED(controller_name_, controller_name_ << ":: watchdog timedout! " << cmd_watchdog_duration_ << " " << (time - cmd_last_stamp_) );
        //} else ROS_WARN_STREAM_NAMED(controller_name_, controller_name_ << ":: watchdog not timedout! " << cmd_watchdog_duration_ << " " << (time - cmd_last_stamp_));
    ...
    void RB1BaseController::elevatorControlLoop(const ros::Time& time){
		...
		// RAISING
			...
			if(elevator_status_.position == robotnik_msgs::ElevatorStatus::UP){
				...
				if( (time - elevator_action_init_time).toSec() > RB1_ELEVATOR_DEFAULT_ELEVATOR_ACTION_TIMEOUT){
					ROS_ERROR("%s::elevatorControlLoop: timeout in elevator RAISING action. %.1f seconds without finishing the action", controller_name_.c_str(), RB1_ELEVATOR_DEFAULT_ELEVATOR_ACTION_TIMEOUT);
		...
		// LOWERING
			...
			if(elevator_status_.position == robotnik_msgs::ElevatorStatus::DOWN){
				...
				// Checking timeout
				if( (time - elevator_action_init_time).toSec() > RB1_ELEVATOR_DEFAULT_ELEVATOR_ACTION_TIMEOUT){
					ROS_ERROR("%s::elevatorControlLoop: timeout in elevator LOWERING action. %.1f seconds without finishing the action", controller_name_.c_str(), RB1_ELEVATOR_DEFAULT_ELEVATOR_ACTION_TIMEOUT);
					switchToElevatorState((string)robotnik_msgs::ElevatorStatus::ERROR_TIMEOUT);
				}
			}
	...
	void RB1BaseController::switchToElevatorState(string new_state){
		if(new_state == elevator_status_.state)
			return;
		ROS_INFO("%s::switchToElevatorState: from %s to %s", controller_name_.c_str(), elevator_status_.state.c_str() ,new_state.c_str());
		
		elevator_status_.state = new_state;
		
		if(elevator_status_.state == robotnik_msgs::ElevatorStatus::ERROR_G_IO){			
			elevator_status_.position = robotnik_msgs::ElevatorStatus::UNKNOWN;	
			ROS_WARN("%s::switchToElevatorState: elevator on error, disabling all the outputs");
			setDigitalOutput(elevator_digital_output_down_, false);
			setDigitalOutput(elevator_digital_output_up_, false);
			
		}else if(elevator_status_.state == robotnik_msgs::ElevatorStatus::IDLE){
			elevator_action_.action = robotnik_msgs::ElevatorAction::NO_ACTION;
		}else if( (elevator_status_.state == robotnik_msgs::ElevatorStatus::LOWERING) or (elevator_status_.state == robotnik_msgs::ElevatorStatus::RAISING) ){
    ...
    void RB1BaseController::limitCommand(double period)
    {
        ...
        if (accel_x > linear_acceleration_limit_) 
	     accel_x = linear_acceleration_limit_;
        if (accel_x < -linear_acceleration_limit_)
             accel_x = -linear_acceleration_limit_;
	
        vx = current_cmd_.linear.x + accel_x * period;

        if (vx > linear_speed_limit_) vx = linear_speed_limit_;
        if (vx < -linear_speed_limit_) vx = -linear_speed_limit_;
    ...
    void RB1BaseController::writeJointCommands()
    {
        ...
        if(elevator_status_.state != robotnik_msgs::ElevatorStatus::IDLE && elevator_status_.state != robotnik_msgs::ElevatorStatus::ERROR_G_IO){
			ROS_WARN_STREAM_THROTTLE_NAMED(2,controller_name_, controller_name_ << "::writeJointCommands: elevator active! disabling commands");
			disable_command = true;
		}
    ...
    void RB1BaseController::updateOdometry()
    {
        ...
        double frw1 = joint_states_[RIGHT_TRACTION_JOINT];
        double flw2 = joint_states_[LEFT_TRACTION_JOINT];
        ...
        double fDistanceBetweenWheels = 0.421; //0.92 -- 22/05/26;
        ...
        if (!imu_watchdog_timedout_) robot_pose_.theta = imu_yaw_;
        else robot_pose_.theta += w * fSamplePeriod;
    ...
    void RB1BaseController::setJointVelocityReferenceBetweenLimits(std::vector<double> &wheel_speed)
    {
